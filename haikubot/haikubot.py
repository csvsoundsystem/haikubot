...
class HaikuBot(object):
  def __init__(
    ...
    # should we post to meatspac.es?
    self.meats = meats

    # twitter status_id db
    if haiku_ids_file is None:
      self.haiku_ids_file = "haiku-status-ids.txt"
    else:
      self.haiku_ids_file = haiku_ids_file

    try:
      status_ids = list(set([
        i for i in open(self.haiku_ids_file).read().split("\n") if i != ""
      ]))
    except:
      status_ids = []
